	...
	void Initialize(py::array_t<float> scene_params, int thread_idx = 0)
	{
		...
		*/
		...
		for (int i = 0; i < num; ++i)
			...
			CreateParticleShape(make_path(torus_path, "/data/torus.obj"), Vec3(lowerx + (i % 3) * torus_height / 3., torus_height * (i + 1), lowerz + (i % 3) * torus_height / 3.), size, 0.0f, radius * 0.5f, Vec3(0.0f, 0.0f, 0.0f), 0.125f, true, 1.0f, NvFlexMakePhase(group++, 0), true, 0.0f);
